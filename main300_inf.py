...
path = './data/freq_day/csi300_Alpha158'
...
for win_len, dropout, num_layers,lr,reg,loss,d_model in parameter_combinations:
    ...
    model = TransformerModel(d_feat = 44,
                             d_model = d_model,
                             batch_size = max(2,int(8 / (num_layers / 4) / (win_len / 10) / (d_model / 256))),#2048*4, 20 - 21
                             nhead = 8,
                             ...
                             model_path = '/home/qiyiyan/linsida_www/log/241031_1445/model_v0.046_t0.034.model'
                             )
    
    # model.tokensize = len(instrument_all)
    # model.fit([train_ds, valid_ds, test_ds])        
    pred_stock,valid_ds, stock_score,ddf = model.predict(valid_ds)  
    ddf.to_parquet(save_path+f'/test_{stock_score, win_len, dropout, num_layers,lr,reg,loss,d_model}.parquet')
    pred_stock,valid_ds, stock_score,ddf = model.predict(test_ds)  
    ddf.to_parquet(save_path+f'/valid_{stock_score, win_len, dropout, num_layers,lr,reg,loss,d_model}.parquet')
    break
